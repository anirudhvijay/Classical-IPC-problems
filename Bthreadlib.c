...
void block()
{
	...
	sigprocmask(SIG_BLOCK,&act1.sa_mask,NULL); //block all signals
}
void unblock()
{
	sigprocmask(SIG_UNBLOCK,&act1.sa_mask,NULL);   //restore blocked signals
...
void schedule(int l)
{
	...
	else
	{
		...
		else
		{
			...
			head = head->next;
			swapcontext(&tail->task,&head->task);
		}
	}
}
void init(struct itimerval it)
